<template>
  ...
  >
    <div class="optionsTable flex justify-between items center">
      <div class="flex">
        <button
          style="min-width: 130px"
          class="btnContentHeader1 mr-5"
          @click="activeDateRange"
        >
          <img src="~/assets/playground_assets/calendarline4865-l7i.svg?url" />
          <span> Date range </span>
        </button>
        <VueDatePicker
          v-show="dateRangeActive"
      ...
      <table class="w-full text-sm text-left">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-4 text-center">
              {{ (row.amount + row.fee_extra).toFixed(2) }}
            </td>
            <td class="px-4 py-4 text-center">{{ row.fee.toFixed(2) }}</td>
            <td class="px-4 py-4 text-center">
              {{ row.net_amount.toFixed(2) }}
            </td>
            <td class="px-4 py-4 text-center">
              {{ row.amount_conversion.toFixed(2) }}
            </td>
            <td class="px-4 py-4 text-center">
              <a
                style="color: rgba(102, 90, 235, 1)"
...
function handleDate(data: any) {
  console.log(data);
  if (data != null) {
    console.log("si hay data entonces emitted", dateRange.value);
    emitters("change-dateRange", dateRange.value, dateRangeActive.value);
  }
}
...
function activeDateRange() {
  dateRangeActive.value = !dateRangeActive.value;
}
